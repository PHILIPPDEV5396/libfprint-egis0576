...
| 4 umockdev test | custom.py written, tooling ready (umockdev, tshark, usbmon), determinism fixes landed (`ae537ab`); blocked on finding a usable non-finger object — the first candidate (rigid metal) enrols but does not verify against itself (coverage gate, placement); `tools/upstream/objprobe.py` checks a candidate in ~20 s | |
